...
function getOptionalInputInt(
  ...
  if (validation?.min && validation.min > result) {
    throw new Error(
      `${name} value (${result}) is less then the allowed minimum (${validation.min})`,
    );
  }

  if (validation?.max && validation.max < result) {
    throw new Error(
      `${name} value (${result}) is greater then the allowed maximum (${validation.max})`,
    );
  }
...
function getInputStr(name: string, defaultValue: string): string {
  return (
    getInput(name, { required: false, trimWhitespace: true }) || defaultValue
  );
}
...
export function getConfig(): IActionParams {
  const rawActionInput = getInput('action', {
    required: false,
    trimWhitespace: true,
  });
...
function getCreateOrUpdateConfig(): ICreateOrUpdateActionParams {
  ...
  const serviceName = getInput('service', {
    required: true,
    trimWhitespace: true,
  });
  ...
  const waitTimeout = getOptionalInputInt(
  ...
  return {
    ...
    environment: getEnvironmentVariables(
      envVarNames,
      getInputStr('replace-string-in-env', ''),
    ),
  };
}
...
function getImageConfig(imageUri: string): IImageConfiguration {
  if (getInput('repo', { required: false })) {
    throw new Error(
      'Either docker image registry or code repository expected, not both',
    );
  }
  return {
    sourceType: 'image',
...
function getRuntime(): Runtime {
  ...
  if (!Object.keys(Runtime).includes(runtime)) {
    throw new Error(
      `Specified runtime (${rawRuntime}) does not belong to the supported range: ${JSON.stringify(
        Object.keys(Runtime),
      )}`,
    );
  }
...
function getEnvironmentVariables(
  ...
  if (envVarNames.length > 0) {
    const mapped = envVarNames.reduce((acc: Record<string, string>, env) => {
      ...
      info(`Finding giving env var key: [${env}], value: [${envVarValue}]`);
